...
2. cast a vote for poll with the `poll_id` that is described in `ref_info`
   ...
   - You probably noticed that there a couple of other fields in `ref_info`
     ...
     - `ref_message_id` - if the link was generated as a result as
       (the flow for this is rather involved, maybe that could be another
       blog post)

3. one JWT payload - the signature `data_check_string`
   ...
   that they weren't invited to vote in. 2. [TODO] ^ this is actually already generated by telegram itself and put into
   headers lol
